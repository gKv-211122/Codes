...
class ABC implements Runnable  {  
	...
	public void run() {  
		...
		System.out.println("The state of thread t1 while it invoked "
				+ "the method join() on thread t2 -"+ ThreadState.t1.getState());  
...
public class ThreadState implements Runnable {  
	...
	public static void main(String argvs[]) {  
		...
		System.out.println("The state of thread t1 after invoking"
		+ " the method start() on it - " + t1.getState());  
	}  
...
public void run()  {  
	...
	System.out.println("the state of thread t2 after"
			+ " calling the method start() on it - " + t2.getState());  
